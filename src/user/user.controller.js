...
export const signUp = async (req, res) => {
    try {
        ...
        if (existingUser) {
            return res.status(400).send({ message: 'Sorry, there is a user already with that name' });
        }
        data.password = await encrypt(data.password)
        let user = new User(data)
        await user.save()
        return res.send({ message: `Registered successfully, can be logged with username ${user.username} or ${user.email}` })
    } catch (err) {
        console.error(err)
...
export const login = async (req, res) => {
    try {
        ...
        if (users && await comparePassword(password, users.password)) {
            ...
            return res.send({ message: `Hi ${loggedUser.name}!`, loggedUser, token })
        }
